...
4. 在AOP类中，通过`Method method = ((MethodSignature) jp.getSignature()).getMethod();`，获取方法元信息
5. 通过`getArgs()`，获取拦截点方法运行时参数
6. 对参数进行处理

---------------------------------------------------------------------------------------------------------------

1. 创建枚举文件，明确数据库操作类型 (这一步操作是为了让编译时出现错误就直接报错，而不是运行的时候才出问题)
```java
...
2. 创建自定义注解类 annotation，然后创建注解类，用于标识需要公共字段自动填充的特定方法
```java
