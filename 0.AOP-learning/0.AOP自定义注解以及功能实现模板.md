...
- 在需要执行AOP方法的业务方法上写上自定义的注解 `@LogOperation`

------------------------------------------------------------------------------------------------------------------------------

项目代码完整AOP实现模板(*苍穹外卖-公共字段自动填充*)：
...
```
	...
	1. 创建自定义注解包 annotation，然后创建注解类，用于标识需要公共字段自动填充的特定方法
```java
			@Target(ElementType.METHOD)				//标记为在方法上生效
