...
- 全局异常处理类的本质就是定义若干 形参 为 各种异常类 的方法
- 抛出异常时，如果异常类或者父类包括在其中，就会通过全局异常来处理和抛出
- 自定义异常继承了`Exception` `RunTimeException`等类后，只要全局异常处理类中的方法参数里，有这些异常的父类，就可以被处理
  ...
  1. 首先需要创建`GlobalExceptionHandler`类:
   ```java
    ...
    public class GlobalExceptionHandler {
        ...
        @ExceptionHandler           // RunTimeException类 也是可以用的
