...
---------------------------------------------------------------------------------------------------------------------------
#### 令牌校验模板代码：
- 预先须知：所有请求以及响应，都会被封装到`HttpServletRequest`和`HttpServletResponse`类中
- 要获取请求路径和令牌，则需要分别调用
```
                                        - request.getRequestURI();
                                        - response.getHeader("token");
```
<div><b>注意：</b>这里仅仅是模板代码，检验(解析)token的判断依据仅仅只有token格式是否合法，以及是否过了有效。而实际的项目中还需要二次检验账号状态等验证</div>
<br>
